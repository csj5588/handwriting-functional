...
// => Container(4) 它返回了一个值为4的Container实例
 ...
 * 操作结束后，为了防止意外再把它放回它所属的 Container。这样做的结果是，我们能连续地调用 map，
 * 运行任何我们想运行的函数。甚至还可以改变值的类型，就像上面最后一个例子中那样。
   ...
   没错，functor 就是一个签了合约的接口。我们本来可以简单地把它称为 Mappable，但这样就没有 fun
   译者注：指 functor 中包含 fun 这个单词，是一双关语）了，对吧？
   functor 是范畴学里的概念，我们将在本章末尾详细探索与此相关的数学知识；暂时我们先用这个名字很奇怪的接口做一些不那么理论的、实用性的练习。
