/**
 * 在关于纯函数的那一章（第三章）里，有一个很奇怪的例子。这个例子中的函数会产生副作用
 * 但是我们通过把它包裹在另一个函数里的方式把它变的看起来像一个纯函数。例子：
 */
...
/**
 * 要是我们没有把getFromStorage包裹在另一个函数里，它的输出值是不确定的，会随着外部环境变化而变化。
 * 有了这个结实的包裹函数（Wrapper）。同一个输入就能总返回同一个输出：一个从localStorage里取出某个特定元素的函数。
 * 就这样我们洗涤里心灵，一切都得到了宽恕。
 * 
...
/**
 * IO 跟之前的 functor 不同的地方在于，它的 __value 总是一个函数。不过我们不把它当作一个函数——实现的细节我们最好先不管。
 * 这里发生的事情跟我们在 getFromStorage 那里看到的一模一样：IO 把非纯执行动作（impure action）捕获到包裹函数里，
 * 目的是延迟执行这个非纯动作。就这一点而言，我们认为 IO 包含的是被包裹的执行动作的返回值，而不是包裹函数本身。
 ...
 * 我把这里的返回值都写成了概念性的，这样就更加直观；不过实际的返回值是 { __value: [Function] }。
 * 当调用 IO 的 map 的时候，我们把传进来的函数放在了 map 函数里的组合的最末端（也就是最左边），反过来这个函数就成为了新的 IO 的新 __value，
 * 并继续下去。传给 map 的函数并没有运行，我们只是把它们压到一个“运行栈”的最末端而已，一个函数紧挨着另一个函数，
 * 就像小心摆放的多米诺骨牌一样，让人不敢轻易推倒。
 * 这种情形很容易叫人联想起“四人帮”（译者注：《设计模式》一书作者）提出的命令模式（command pattern）或者队列（queue）。

   花点时间找回你关于 functor 的直觉吧。把实现细节放在一边不管，你应该就能自然而然地对各种各样的容器使用 map 了，
   不管它是多么奇特怪异。这种伪超自然的力量要归功于 functor 的定律，我们将在本章末尾对此作一番探索。
   ...
   答案是可以的，只要把责任推到调用者身上就行了。我们的纯代码，尽管阴险狡诈诡计多端，但是却始终保持一副清白无辜的模样，
   反而是实际运行 IO 并产生了作用的调用者，背了黑锅。来看一个具体的例子。
 */
...
var findParams = function(key) {
  return map(comopse(Maybe.of, filter(compose(eq(key, head)), params), url))
}

////// 非纯调用代码: main.js //////

// 调用 _value() 来运行它！
findParams('searchTerm')._value();
