 ...
 * "纯错误处理"
 * 
 * 说出来可能会让你震惊，throw/catch 并不十分“纯”。当一个错误抛出的时候，我们没有收到返回值，
 * 反而是得到了一个警告！抛错的函数吐出一大堆的 0 和 1 作为盾和矛来攻击我们，简直就像是在反击输入值的入侵而进行的一场电子大作战。
...
/**
 * Left 和 Right 是我们称之为Either的抽象类型的两个子类。
 * 我略去了创建Either父类的繁重，因为我们不会用到他的，但你了解一下也没坏处。
 * 注意看这里除了有两个类型，没别的新鲜的东西，没别的新鲜东西
 * 来看看它们是怎么运行的。
 */
...
/**
 * 这么一来，就像Maybe（null），当返回一个Left当时候就直接让程序短路。
 * 跟Maybe（null）不同的是，现在我们对程序为何脱离原先轨道至少有了一点头绪
 * 有一件事要注意，这里返回的是Either（String， Number），意味着我们这个Either左边的值是String，
 * 正确的值是Number。这个类型签名不是很正式，因为我们并没有定义一个真正的Either父类；
 * 但是我们还是从这个类型那里了解到不少东西。它告诉我们，我们得到的要么是一条错误的消息，要么是正确的年龄值。
 */
...
/**
 * 如果 birthdate 合法，这个程序就会把它神秘的命运打印在屏幕上让我们见证；
 * 如果不合法，我们就会收到一个有着清清楚楚的错误消息的 Left，尽管这个消息是稳稳当当地待在它的容器里的。
 * 这种行为就像，虽然我们在抛错，但是是以一种平静温和的方式抛错，而不是像一个小孩子那样，有什么不对劲就闹脾气大喊大叫。
 * 
 ...
 * 我们在 Right 分支的类型签名中使用 _ 表示一个应该忽略的值（在有些浏览器中，你必须要 console.log.bind(console) 才能把 console.log 当作一等公民使用）。
   ...
   前面例子中的 finishTransaction 也是一样。通俗点来讲，一个函数在调用的时候，如果被 map 包裹了，那么它就会从一个非 functor 函数转换为一个 functor 函数。
   ...
   此刻我忍不住在想，我仅仅是把 Either 当作一个错误消息的容器介绍给你！这样的介绍有失偏颇，它的能耐远不止于此。比如，它表示了逻辑或（也就是 ||）。
   再比如，它体现了范畴学里 coproduct 的概念，当然本书不会涉及这方面的知识，但值得你去深入了解，因为这个概念有很多特性值得利用。
   还比如，它是标准的 sum type（或者叫不交并集，disjoint union of sets），因为它含有的所有可能的值的总数就是它包含的那两种类型的总数（
   我知道这么说你听不懂，没关系，这里有一篇非常棒的文章讲述这个问题）。Either 能做的事情多着呢，但是作为一个 functor，我们就用它处理错误。
